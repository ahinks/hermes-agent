...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  try {
    ...
    rememberLog(
      `Failed to persist connection.json for profile "${profile}": ${writeErr instanceof Error ? writeErr.message : String(writeErr)}`
    )
  }
...
async function runHermesStart() {
  ...
  const connectionPromise = (async () => {
    ...
    try {
      ...
      rememberLog(
        `Failed to persist connection.json on primary boot: ${writeErr instanceof Error ? writeErr.message : String(writeErr)}`
      )
    }
